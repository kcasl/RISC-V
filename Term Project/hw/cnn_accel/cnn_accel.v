...
always@(posedge clk, negedge rstn)begin
    if(~rstn) begin	
		...
		if(data_vld_o_ld) begin
			// [FILLED] Store DMA-loaded LR input pixel
...
generate
    ...
    for (i=0; i<To; i=i+1) begin: u_conv_kern
		// [FIX] Packed-bus part-selects (Vivado rejects scale[i]/win[i]/acc_o[i] width mismatch)
...
// Outputs
//-------------------------------------------------
// [FIX] Pack first 4 output channels as bytes (not single bits)
assign out_pixel = {acc_o[3*ACT_BITS+:ACT_BITS],
					acc_o[2*ACT_BITS+:ACT_BITS],
					acc_o[1*ACT_BITS+:ACT_BITS],
					acc_o[0*ACT_BITS+:ACT_BITS]};
