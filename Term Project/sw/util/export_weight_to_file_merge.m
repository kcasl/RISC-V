...
function export_weight_to_file_merge(weight, Ti, To, file_id, bitwidth)
                ...
                for o = (no - 1)*To + 1 : no*To
                    for i =  (ni - 1)*Ti + 1 : ni*Ti
                        if i > ci*f1*f2 || o > co
                            ...
                            k = mod(i-1,f2)+1;
                            n = mod((i-k)/f2,f1)+1;
                            m = (i-k-(n-1)*f2)/f1/f2+1;
%                             fprintf('i= %d m=%d, n=%d, k=%d\n', i, m,n,k);
                                        w = dec2hex(weight(n,k,m,o),2);
                        end
...
% [FIX] size(A) returns [rows cols]; colon needs a scalar → use size(A,1)
for i = 1:nline
    fprintf(file_id,'%s\n',weight_line(i,:));
end
